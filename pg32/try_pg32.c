...
// program
// ./t pg32.txt pg32_inductive.v 15 35
...
/* improved false implementation
int Pinter(int tab[][PPL], int l1, int l2, int points_per_line, int p, int l)
{
  int i,t=1;
  for(i=0;i<p&&t;i++)
    if (belongs(tab, l1, points_per_line, i)&&(belongs(tab, l2, points_per_line,i))) {t=0;}//break;}
  if (t!=0) return (-1); else return i;
}
*/

/*
int Linter(int tab[][PPL], int p1, int p2, int points_per_line, int p, int l)
{
  int i,j,t=1;
  for(i=0;i<l&&t;i++)
    for(j=i+1; j<l&&t;j++)
    if (belongs(tab, i, points_per_line, p1)&&(belongs(tab, j, points_per_line,p2))) {break;}
  return i;
}
*/
s_a3 find_v4(int tab[][PPL], int l1, int l2, int l3, int points_per_line, int p, int l)
{
  int t=1,xl;
  s_a3 r;
  //  printf("PPL = %d, points_per_line =%d\n", PPL, points_per_line);
  for (xl=0;xl<l&&t==1;xl++)
	...
	{
	  printf("xl (%d) = %d %d %d \n", xl ,tab[xl][0], tab[xl][1], tab[xl][2]);
	    printf("l1 (%d) = %d %d %d \n", l1 ,tab[l1][0], tab[l1][1], tab[l1][2]);
	    printf("l2 (%d) = %d %d %d \n", l2 ,tab[l2][0], tab[l2][1], tab[l2][2]);
	    printf("l3 (%d) = %d %d %d \n", l3 ,tab[l3][0], tab[l3][1], tab[l3][2]);
	    
	    printf("-- P1 = %d, P2 = %d, P3 = %d --\n",r.p1, r.p2, r.p3);
	    //lineP1P2P3(tab, p1,p2,p3, 4, p, l);
	    printf(" -- an appropriate line is %d [ %d %d %d ]\n\n", xl,
	    tab[xl][0], tab[xl][1], tab[xl][2]);
	    //fprintf(f, "(%d,%d,%d) |-> ((o %d %d),((o %d %d), (o %d %d), (o %d %d)))",l1,l2,l3,l,xl,p,p1,p,p2,p,p3);
	    //if((l3<l-1)||(l2<l-1)||(l1<l-1)) fprintf(f,",\n"); else fprintf(f,"\n");
	    //break;
	    t=0;
	    //assert(l1!=0||l2!=1||l3!=7);
	} 
    }
  r.v4=xl-1;
...
void build_incidence_relation(FILE *f, char *in, int p, int l)
{
  ...
  // collineation L0, L19, L24, L28, L33 -> L0, L19, L26, L29, L32

  printf("all spreads of pg(3,2)... \n");
  // find spreads
  int m1,m2,m3,m4,m5,m;
  int *part = (int *) malloc(sizeof(int[p]));
  //int nb=1;
  int somme=0;
  int nb_spreads=0;
  int (*spreads)[5] = malloc(sizeof(int[56][5]));
	...
	for(m4=m3+1;m4<l;m4++)
	  for(m5=m4+1;m5<l;m5++)
	    {somme =0;
	      ...
	      // a spread of pg(3,q) is a set of q^2+1 lines disjoint
	      //	      is_spread(m1,m2,m3,m4,m5);
		...
		{
		  t=1;
		  if((m!=m1)&&(m!=m2)&&(m!=m3)&&(m!=m4)&&(m!=m5))
		    {
		      //printf(" new line to check : %d, its points are %d %d %d. \n", m,
		      //	     tab[m][0], tab[m][1], tab[m][2]);
		      if((part[tab[m][0]]==1)||(part[tab[m][1]]==1)||(part[tab[m][2]]==1))
			{t=0;}
		    }
		}
	      if(t==1)
  ...
  printf("nb_spreads = %d\n", nb_spreads);
  // display spreads !
  FILE *g=fopen("spreads_pg32.txt","w");
  for(i=0;i<nb_spreads /*56*/;i++)
    {
      fprintf(g, "spread #%d : %d %d %d %d %d\n", i,spreads[i][0],spreads[i][1],spreads[i][2],spreads[i][3],spreads[i][4]);
      printf("spread #%d : %d %d %d %d %d\n", i,spreads[i][0],spreads[i][1],spreads[i][2],spreads[i][3],spreads[i][4]);
    }
  fclose(g);

  // find packings
  printf("all packings of pg(3,2)... \n");
  int p1,p2,p3,p4,p5,p6,p7;
  int *part2 = (int *) malloc(sizeof(int[l]));
  int somme2=0;
  int nb_packings=0;
  int (*packings)[7] = malloc(sizeof(int[240][7])); //240
